...
sl_orders        = spark.read.table("lakehouse_olist.sl_orders")
sl_order_items   = spark.read.table("lakehouse_olist.sl_order_items")
sl_customers     = spark.read.table("lakehouse_olist.sl_customers")
sl_sellers       = spark.read.table("lakehouse_olist.sl_seller")
sl_geolocation   = spark.read.table("lakehouse_olist.sl_geolocation")
sl_products = spark.read.table("lakehouse_olist.sl_products")
sl_reviews = spark.read.table("lakehouse_olist.sl_reviews")
sl_payments = spark.read.table("lakehouse_olist.sl_payments")
...
# sl_products.printSchema()

sl_products.limit(3).show()
...
sl_products = sl_products.select(
...
# distinct customer_id - sl_customers.select("customer_id").distinct().count()
sl_customers.show(5)
...
unmatched_zips = sl_customers.select("customer_zip_code_prefix").distinct() \
    .join(
        sl_geolocation.select("geolocation_zip_code_prefix").distinct(),
        sl_customers.customer_zip_code_prefix == sl_geolocation.geolocation_zip_code_prefix,
        how="left_anti"  # Returns ZIPs from customers NOT in geolocation
    )

unmatched_count = unmatched_zips.count()
total_unique_zips = sl_customers.select("customer_zip_code_prefix").distinct().count()
...
dim_customer = sl_customers.join(
...
source_count = sl_customers.count()
...
sl_sellers.select("seller_id").distinct().count()
...
dim_seller = sl_sellers.join(
    broadcast(sl_geolocation),
    sl_sellers.seller_zip_code_prefix == sl_geolocation.geolocation_zip_code_prefix,
    how="left" # left join allows nulls for missing geolocaiton data
) \
...
gd_dim_customer = spark.read.table("lakehouse_olist.gd_dim_customer")
...
gd_fact_order_fulfillment = spark.read.table("lakehouse_olist.gd_fact_order_fulfillment")
...
# NOTE: These lost reviews are ORPHANED - their order_ids don't exist in sl_orders
...
# Count reviews
src_count = sl_reviews.count()
print(f"📊 Total reviews in sl_reviews: {src_count:,}")
...
lost_reviews = sl_reviews.join(
    fact_order_fulfillment_lookup.select("order_id"),
    on="order_id",
    how="left_anti"  # Keep rows from sl_reviews that DON'T match fulfillment
)
...
# Check if these orders exist in sl_orders
sl_orders = spark.read.table("lakehouse_olist.sl_orders")
orders_in_source = lost_reviews.join(
    sl_orders.select("order_id"),
    on="order_id",
    how="inner"
).count()

print(f"\n🔎 Of the {loss:,} lost reviews:")
print(f"   - {orders_in_source:,} have matching orders in sl_orders")
print(f"   - {loss - orders_in_source:,} don't even exist in sl_orders (orphaned!)")

# Check order status of lost reviews
if orders_in_source > 0:
    lost_with_status = lost_reviews.join(
        sl_orders.select("order_id", "order_status"),
        on="order_id",
        how="inner"
    )
...
missing_orders = sl_order_items.join(sl_orders, "order_id", "left_anti").count()
missing_products = sl_order_items.join(gd_dim_product, "product_id", "left_anti").count()
missing_sellers = sl_order_items.join(gd_dim_seller, "seller_id", "left_anti").count()
...
qr_missing_orders = sl_order_items.join(sl_orders, "order_id", "left_anti")
qr_missing_products = sl_order_items.join(gd_dim_product, "product_id", "left_anti")
